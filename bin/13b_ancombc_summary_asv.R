...
heatmap_x_label = substr(unique(only_signif_filtered_melted_merged_ancombc_results$taxon),1, 5)
...
ggplot(only_signif_filtered_melted_merged_ancombc_results, aes(x=taxon, y=variable, fill=value_signif)) +
  ...
  labs(fill="LFC", title=paste("ANCOM-BC:", str_replace_all(var_name, "_", " "), "vs", str_replace_all(ref_value, "_", " "
  ), sep=" ")) +
  ...
  scale_x_discrete(labels=heatmap_x_label) +
  scale_y_discrete(labels=heatmap_y_label) +
  geom_text(aes(label=sb_signif), size=4)
ggsave(paste(ancombc_heatmap, ".svg", sep=""), device="svg", width=22, height=4)
ggsave(paste(ancombc_heatmap, ".png", sep="" ), device="png", width=22, height=4)
