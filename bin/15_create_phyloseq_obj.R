...
create_phyloseq_obj <- function(raw_asv_table, asv_table_phyloseq, raw_metadata, metadata_phyloseq, tree, phyloseq_rds, db) {
    #Input data
    ...
    if(db == "silva") {
      ...
      tax = data.frame(tax$ASV_ID,do.call(rbind,list(str_split_fixed(tax$taxonomy, ";",8))))
      colnames(tax) = c("ASV_ID","Domain","Supergroup","Division","Class","Order","Family","Genus","Species")
    } else if(db == "pr2-5") {
      tax = data.frame(tax$ASV_ID,do.call(rbind,list(str_split_fixed(tax$taxonomy, ";",9))))
      colnames(tax) = c("ASV_ID","Domain","Supergroup","Division","Subdivision","Class","Order","Family","Genus","Species")
    }
    ...
    if(db == "silva") {
      PHYLOSEQ = subset_taxa(PHYLOSEQ, Kingdom != "Unassigned" | Kingdom != "")
    } else if(db == "pr2-4" || db == "pr2-5") {
      PHYLOSEQ = subset_taxa(PHYLOSEQ, Domain != "Unassigned" | Domain != "")
    }
...
main <- function() {
    ...
    phyloseq_rds = args[6]
    db = args[7]
    create_phyloseq_obj(raw_asv_table, asv_table_phyloseq, raw_metadata, metadata_phyloseq, tree, phyloseq_rds, db)
}
