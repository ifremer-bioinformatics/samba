...
#    ggsave(filename=ASV_ordination_plot_wrapped_css,width=13,height=10)
    ...
    plot_ordination(PHYLOSEQ_css,ord_css,type="samples",color=criteria) +
      theme_classic() +
      geom_point(size=4) +
      ...
      labs(caption = paste("Stress:",round(ord_css$stress,4),
          "\nANOSIM statistic R:",round(anosim_result_css$statistic,4),
          paste("\nAnosim based on ",criteria," : p-value"),anosim_result_css$signif,sep=" "))
