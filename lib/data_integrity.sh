...
[ ! ${single_end} ] && COL=`head -n1 ${manifest} | tr "\t" "\n" | grep -n ${R2_files} | cut -d ":" -f1`
[ ! ${single_end} ] && cut -f$COL ${manifest} | sed '1d' > tmp_R2
...
[ ! ${single_end} ] && paste -d ' ' tmp_barcodes tmp_R2 | sed "s/^/zgrep -c ':N:0:/g" | sed "s/ /' /3" | sh > tmp_barcodes_count_R2
paste -d ' ' tmp_sequencer tmp_R1 | sed "s/^/zgrep -c '/g" | sed "s/ /' /3" | sh > tmp_sequencer_count_R1
[ ! ${single_end} ] && paste -d ' ' tmp_sequencer tmp_R2 | sed "s/^/zgrep -c '/g" | sed "s/ /' /3" | sh > tmp_sequencer_count_R2
for i in $(cat tmp_R1) ; do zgrep -c "${primerF}" ${i} >>  tmp_primer_count_R1 ; done
[ ! ${single_end} ] && for i in $(cat tmp_R2) ; do zgrep -c "${primerR}" ${i} >>  tmp_primer_count_R2 ; done
[ ! ${single_end} ] && paste -d '      ' tmp_sampleID tmp_barcodes tmp_reads tmp_barcodes_count_R1 tmp_barcodes_count_R2 tmp_sequencer_count_R1 tmp_sequencer_count_R2 tmp_primer_count_R1 tmp_primer_count_R2 > tmp_output
[ ${single_end} ] && paste -d '	' tmp_sampleID tmp_barcodes tmp_reads tmp_barcodes_count_R1 tmp_sequencer_count_R1 tmp_primer_count_R1 > tmp_output
...
[ ! ${single_end} ] && valcol=6;addcol=12 #column = Perc_uniq_sequencer_R1
awk -v OFS='\t' -v "newcol=${addcol}" -v "col=${valcol}" '{$newcol = ($col != 0) ? sprintf("%.0f", $col/$3*100) : "0"}1' tmp_output | sort -nr -k ${addcol} > tmp_output.tmp && mv tmp_output.tmp tmp_output
...
[ ! ${single_end} ] && awk -v OFS='\t' -v "newcol=$(($addcol+1))" -v "col=$(($valcol+1))" '{$newcol = ($col != 0) ? sprintf("%.0f", $col/$3*100) : "0"}1' tmp_output | sort -nr -k $(($addcol+1)) > tmp_output.tmp && mv tmp_output.tmp tmp_output
[ ! ${single_end} ] && if [ $(tail -n 1 tmp_output | awk -F "\t" -v "newcol=$(($addcol+1))" '{print $newcol}') -ge 100 ] ; then touch sequencer_R2.ok ; else touch sequencer_R2.bad ; fi
...
[ ! ${single_end} ] && if [[ -e barcodes_R1.ok && -e barcodes_R2.ok && -e sequencer_R1.ok && -e sequencer_R2.ok && -e primer_R1.ok && -e primer_R2.ok ]] ; then touch ${verif_ok} ; else touch ${verif_bad} && echo '@@@ --- All verifications are not satisfied --- @@@' ; fi
mv tmp_output ${summary}
[ ${single_end} ] && sed -i '1 i\SampleID\tBarcode\tReads_count\tCount_in_R1\tSequencer_R1\tPrimerF_in_R1\tPerc_correct_barcode_R1\tPerc_uniq_sequencer_R1\tPerc_primerF_R1' ${summary}
[ ! ${single_end} ] && sed -i '1 i\SampleID\tBarcode\tReads_count\tCount_in_R1\tCount_in_R2\tSequencer_R1\tSequencer_R2\tPrimerF_in_R1\tPrimerR_in_R2\tPerc_correct_barcode_R1\tPerc_correct_barcode_R2\tPerc_uniq_sequencer_R1\tPerc_uniq_sequencer_R2\tPerc_primerF_R1\tPerc_primerR_R2' ${summary}
rm tmp*
